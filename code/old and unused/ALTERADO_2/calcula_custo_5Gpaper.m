function [ costs ] = calcula_custo_5Gpaper( usuario, micro, tunelamento, passo)
Value_micro=1600;
...
%Checar maior vazao
for h=1:length(microTESTE)
    uncoveredTESTE = 0;
    vazao_total=0;
    microTESTE(h).deployed = false;
    microTESTE(h).check = 1;
    [usuarioTESTE microTESTE uncoveredTESTE] = updateScenario(usuarioTESTE, microTESTE, frequency);
    for j=1:length(usuarioTESTE)
        vazao_total = vazao_total + usuarioTESTE(h).dataRate;
    end
    microTESTE(h).deployed = true;
    microTESTE(h).vazaoTotal = vazao_total;
end
...
%dbstop in calcula_tunelamento2        
        ...
        costs(m,10) = microTESTE(micro(i).id).vazaoTotal;
        m = m + 1;
   end
end

 if auxcost == true
        costs = sortrows(costs,-10);
 else
    costs = [];
 end
end