function [total, tot_fiber, tot_trenching ,sub]=calcula_trenching(implantados,pontos,passo)
...
for i=1:1:size(newpoint3,1)
    ...
    for j=1:1:size(pontos,1);
        ...
        costs(i,1)=Value_Cost/newpoint3(i,3)+((taxicab(1)*fibra+taxicab(1)*cano));
...
tot_fiber = sum(implantados(:,8));
tot_trenching = sum(implantados(:,7));
total= sub + tot_fiber + tot_trenching;
%########################################################################
%########################################################################