function [ usuario, micro, custo_final] = selectionBs( usuario, micro, frequency, passo, l )
x = 0;
y = 0;
tunelamento=[];
...
[ costs ] = calcula_custo( usuarioT, microT, tunelamento, passo);
...
i=1;
custo_final = 0;
while (i <= length(costs))
    if (microT(costs(i,9)).check == 0)
        for z = 1:length(usuarioT)
