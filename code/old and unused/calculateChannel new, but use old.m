...
L_db = 92.4 + 20 * log10(distance) + 20 * log10(frequency/1e9) + L_Foliage; % frequency conversion from Hz to GHz
...
for i=1:length(micro) % calculate intercell interference
  ...
  if(micro(i).deployed && micro(i).id ~= antenna.id)
    ...
    L_dbA = micro(i).radiated_power - (92.4 + 20 * log10(distanceA) + 20*log10(frequency/1e9)+ L_Foliage);
